...
void fusionarArchivos(const char *archivoDestino, const char *archivos[], int numArchivos) {
    FILE *outputFile = fopen(archivoDestino, "wb");
    if (outputFile == NULL) {
    ...
    for (int i = 0; i < numArchivos; i++) {
        FILE *inputFile = fopen(archivos[i], "rb");
        if (inputFile == NULL) {
    ...
    fclose(outputFile);
    printf("Los archivos han sido fusionados en %s\n", archivoDestino);
}

// Implementación de las demás funciones como ingresarProducto, editarProducto, etc.
